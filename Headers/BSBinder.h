 ...
 * fulfilled. Here's an example of a configure: method on a simple BSModule,
 * adding one binding:
 *
 * \code
 ...
 *    // Any injected onjects that need a MyApi will have this one given to them
 *    // by Blindside.
 *
 ...
 * to bind to a specific object implementing the protocol.
 *
 * When an injector is asked to retrieve an object, it examines the object's dependencies and 
 * then looks at the bindinds defined as above to determine exactly how to fill the
...
- (void)bind:(id)key toInstance:(id)instance;
   ...
   * to the provider for it to use as appropriate.
   */
- (void)bind:(id)key toProvider:(id<BSProvider>)provider;
   ...
   *
   * If any arguments are passed to [injector getInstance:], these arguments will be passed
   * to the block for it to use as appropriate.
   * 
   * This trivial example shows how block binding works:
   *
   * \code
   * @implementation MyModule
   * - (void)configure(id<BSBinder>binder {
   ...
   *
   *   [binder bind:@"fullName" toBlock:^id()(NSArray *args){
...
- (void)bind:(id)key toBlock:(BSBlock)block;
   ...
   *
   * If no bindings have been created using [Foo class] a key, then Blindside will try to 
